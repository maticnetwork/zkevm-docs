...
description: 'ExitUtil sınıfı ile kanıt üretir.'
---

`matic.js` kanıt üretmek için dâhili olarak `ExitUtil` kullanır. Çıkış programlarına yardımcı olmak için farklı metotları olan bir sınıftır.

## buildPayloadForExit {#buildpayloadforexit}

Kanıt üretmek için kullanılabilen `buildPayloadForExit` metodunu ortaya çıkarır.
...
### Köprü istemcisi kullanarak kanıt üretme {#generating-proof-using-bridge-client}

**POSClient**, **PlasmaClient** dâhil her köprü istemcisi `exitUtil` özelliğini ortaya çıkarır.
