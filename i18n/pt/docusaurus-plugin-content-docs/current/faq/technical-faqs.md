...
Acompanhe as atualizações mais recentes de nós e validadores da equipe do Polygon e da comunidade assinando [<ins>notificações do Polygon</ins>](https://polygon.technology/notifications/).
...
Sim, a chave privada utilizada para gerar chaves de Validador e o repositório de chaves do Bor são os mesmos. A chave privada utilizada neste caso é a do endereço de ETH da sua carteira, onde os seus tokens testnet da Polygon são armazenados.

### 2. Lista de Comandos Comuns {#2-list-of-common-commands}

Atualmente, temos uma lista de fácil consulta para si, referente aos pacotes Linux. Continuaremos atualizando esta lista regularmente para maior conveniência.
...
### 4. Error: Wrong Block.Header.AppHash. xxxx esperado {#4-error-wrong-block-header-apphash-expected-xxxx}

Este erro geralmente ocorre quando o serviço da Heimdall está preso em um bloco; não existem métodos de reversão disponíveis na Heimdall.

Para resolver isso, precisará repor a Heimdall completamente:
...
Depois disso, deve sincronizar a partir do snapshot novamente:
...
Então, inicie os serviços da Heimdall novamente.
...
Altere a config para `goleveldb`em config.toml
...
**Error**: "dpkg: error processing archive matic-heimdall_1.0.0_amd64.deb (--install): trying to overwrite '/heimdalld-rest-server.service', que também está no pacote matic-node 1.0.0"

Isto ocorre principalmente devido a uma instalação anterior do MATIC em sua máquina. Para resolver, pode executar:
...
Excluir a Heimdall: `sudo rm -rf /var/lib/heimdalld/`
